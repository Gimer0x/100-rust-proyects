...
// Build a tool that compares two directories and copies new or changed files 
// from a "source" to a "destination", simulating one-way file synchronization. 
// This project teaches file system traversal, metadata comparison, and file 
// copying—great for backup and sync systems.
// Key Concepts:
// + fs::read_dir for directory traversal
// + fs::copy for file copying
// + metadata.modified() for timestamp comparison
// You've now built a core component of backup utilities, sync clients, and 
// deployment tools that handle file mirroring and change detection.
use std::fs;
use std::io;
